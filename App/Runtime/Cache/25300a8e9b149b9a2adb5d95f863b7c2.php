				...
				<div class="ibox-content" id="center_height" style="background-color:#fff;">
                    <div class="full-height-scroll">
    					<div class="ibox-content ibox-title" style="background-color:#fff;padding-bottom:0px;border: none;padding-top: 5px;">
                            <a href="<?php echo U('User/department');?>">
                           ...
                           <div style="font-size:13px;background-color:#fff;" id="tree1"><?php echo ($department_tree); ?></div>
                        </div>
                    </div>
				</div>
			</div>
			<div class="col-md-7" style="padding-left: 0px;">
                <div class="ibox-content" id="right_height" style="overflow-x:auto;float:left;background-color:#fff;min-width:100%;">
                    <div class="full-height-scroll">
        				...
        				<p style="text-align:center;padding-top: 21px;"><a data-toggle="modal" data-target="#Modal" data-backdrop="static" href="<?php echo U('user/position_create?department_id='.$_REQUEST['department_id']);?>"><i class="fa fa-plus"></i>&nbsp;点击创建</a></p>
        				<p style="text-align:center;padding-top: 10px;">当前部门岗位为空!</p>
        				<?php else: ?>
        				<div style="font-size:13px;background-color:#fff;" id="tree2" style="">
        					<?php echo ($position_tree); ?>
        				</div>
        				<?php endif; ?>
                        </div>
                    </div>
...
function depTwo(e){
    ...
    switch(type){
        case 'edit':
            var url = "<?php echo U('user/position_edit');?>&id="+position_id+'&department_id='+department_id;
          break;
        case 'addUser':
            var url = "<?php echo U('user/user_add');?>&id="+position_id+'&department_id='+department_id;
          break;
        case 'addUp':
            var url = "<?php echo U('user/position_up');?>&id="+position_id+'&department_id='+department_id;

            $.get("<?php echo U('user/is_parent');?>", {'id':position_id}, function(msg){
                if(msg == 1){
                    swal({
                        title: "温馨提示",
                        text: "顶级岗位不能插入上级！",
          ...
          break;
        case 'addDown':
            var url = "<?php echo U('user/position_down');?>&id="+position_id+'&department_id='+department_id;
          break;
        case 'addAuth':
            $("#dep_modal").parent().removeClass("modal-md").addClass("modal-lg");
            var url = "<?php echo U('user/user_authorize');?>&position_id="+position_id+'&department_id='+department_id;
          break;
        //case 'copyAuth':
        // var url = "<?php echo U('user/department_edit');?>&id="+position_id+'&department_id='+department_id;
        // break;
        case 'del':
            var url = "<?php echo U('user/position_del');?>&id="+position_id+'&department_id='+department_id;
            swal({
                    title: "您确定要删除此岗位吗？",
                    text: "删除后将无法恢复，请谨慎操作！",
                    type: "warning",   
                ...
                function(isConfirm){
                    if (isConfirm) {
                    //window.location="<?php echo U('user/department_delete');?>&id="+dep_id;
                        $.ajax({
                            type: "GET",
                            url: url,
                            async: true,
...
function depEdit(obj){
    ...
    $url = "<?php echo U('user/department_edit');?>&id="+dep_id;
...
function depAdd(obj){
    $("#dep_modal").parent().removeClass("modal-lg").addClass("modal-md");
    var dep_id = $(obj).parent().parent().attr('rel');
    $url = "<?php echo U('user/department_add');?>&id="+dep_id;
    $('#Modal').modal('show');
    $('#dep_modal').load($url);
}
...
function depMove(obj){
    ...
    $url = "<?php echo U('user/department_edit');?>&move=1&id="+dep_id;
    $('#Modal').modal('show');
    $('#dep_modal').load($url);
}
    ...
    swal({
        ...
        function(isConfirm){
            if (isConfirm) {
                ...
                $.ajax({
                    type: "GET",
                    url: "<?php echo U('user/department_delete');?>&id="+dep_id,
                    async: true,
                    success: function(data) {
                        if(data.status == 1){
                            swal({
                            ...
                            $('#Modal').modal('hide');
                            window.location.href="<?php echo U('department');?>";
                            //location.reload();
