...
<body class="navbar">
 ...
 $img = M('User')->where('user_id = %d', session('user_id'))->getField('img'); $defaultinfo_info = M('Config')->where('name = "defaultinfo"')->find(); $defaultinfo = unserialize($defaultinfo_info['value']); ?>
					...
					<li>
						<a data-toggle="tooltip" data-placement="right" data-original-title="积分业绩" <?php if(($module_name == 'integral')): ?>class="active"<?php endif; ?> href="<?php echo U('integral/index');?>"><i class="fa fa-star"></i><span class="menu_code">积分</span></a>
					</li>
...
<div class="wrapper wrapper-content animated fadeIn col-md-6">
	<form class="form-horizontal" id="form" role="form" action="<?php echo U('customer/edit', 'id='.$customer['customer_id']);?>" method="post">
						...
						<div class="col-md-11 add_body_form">
							<?php if(is_array($field_list['main'])): $key = 0; $__LIST__ = $field_list['main'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key; if($vo['field'] != 'customer_owner_id'): ?><div class="form-group">
										...
										<?php if($vo['form_type'] == 'textarea'): if($vo['tip_start'] == 1): ?><div class="col-md-6">
											...
											<?php if($vo['field'] == 'grade'): ?><div class="col-md-8">
													<input type="hidden" id="star" name="grade" value="<?php echo ($customer['grade']); ?>">
											...
											<div class="col-md-6">
												<input class="form-control required" name="name" onkeyup="checkinfo(name)" value="<?php echo ($customer['name']); ?>" type="text"/>
											</div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <!--<input type="hidden" name="customer_owner_id" id="user_id" value="<?php echo ($vo["customer_id"]); ?>" />-->
                                    <input class="form-control required" aria-required="true" readonly="readonly"  type="text" name="customer_owner_name" id="user_name" value="<?php echo ($customer_owner_name); ?>" readonly="true" style="cursor:pointer;" />
                                </div>
                                <div class="col-md-2"><span style="color:red;line-height: 32px;margin-left: 10px;">*</span></div>
                            </div>
						</div>
