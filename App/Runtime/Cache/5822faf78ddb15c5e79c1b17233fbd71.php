				...
				<div class="table-content" style="padding-bottom:10px">
					<div class="ibox-content mar-top" style="border-bottom:none;padding-top:20px;">
						<div class="bulk-actions align-left">
							<button onclick="del_status(this)" style="background:#fff;border:1px solid #e3e3e3;color:#000;" type="button" class="btn btn-primary" >&nbsp;<?php echo L('DELETE');?></button>
									...
									<tbody>
										<?php if(is_array($bank_list)): $i = 0; $__LIST__ = $bank_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
												<td style="text-align:center;">
													<div class="checkbox checkbox-primary">
														<input name="account_id[]" class="check_list" type="checkbox" value="<?php echo ($vo["account_id"]); ?>"/>
														<label for=""></label>
													</div>
												</td>
												...
												<td>
													<a class="edit" href="javascript:void(0)" rel="<?php echo ($vo["account_id"]); ?>"><?php echo L('EDIT');?></a>&nbsp;&nbsp;
													<!-- <a class="initialize" href="javascript:void(0)" rel="<?php echo ($vo["account_id"]); ?>">初始化余额</a> -->
												</td>
											</tr><?php endforeach; endif; else: echo "" ;endif; ?>
									</tbody>
									<?php else: ?>
										<tr>
											<td><div style="text-align:center;color:#E4E4E4;font-size:22px;font-weight:700;padding-top:15px;">
...
<div class="" style="display:none;" id="dialog-initialize" title="初始化金额">
	<input type="hidden" name="account_id" id="dialog_account_id" value="<?php echo ($_GET['account_id']); ?>" />
	<table class="table">
		<tr>
...
$(function(){
	...
	$("#add").click(function(){
		...
		$('#dialog-add').load('<?php echo U("setting/category_add");?>');
	});

	$(".edit").click(function(){
		var id = $(this).attr('rel');
		$('#dialog-edit').dialog('open');
		$('#dialog-edit').load('<?php echo U("setting/category_edit","id=");?>'+id);
	});
	...
	if(id_array.length == 0){
		...
		function(isConfirm){
	        if (isConfirm) {
				$.ajax({
					type: "POST",
					url: "<?php echo U('setting/category_delete');?>",
					...
					success: function(data) {
						if(data.status == 1){
							...
							window.location.href="<?php echo U('setting/category');?>";
						}else{
							 swal({
								title: "操作失败!",
								text: data.info,
...
function next_form(obj){
	...
	$.ajax({
		...
		url: "<?php echo U('setting/verify_identity');?>",
		async: true,
		success: function(data) {
			if(data.status == 1){
				$('#next_td').hide();
...
function initialize_form(obj){
	...
	$.ajax({
		type: "POST",
		url: "<?php echo U('setting/initialize');?>",
