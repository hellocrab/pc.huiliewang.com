				...
				<div class="table-content" style="padding-bottom:10px">
					<div class="ibox-content mar-top" style="border-bottom:none;padding-top:20px;">
						<div class="bulk-actions align-left">
							<button onclick="del_category(this)" style="background:#fff;border:1px solid #e3e3e3;color:#000;" type="button" class="btn btn-primary" >&nbsp;<?php echo L('DELETE');?></button>
								...
								<table class="table table-hover table-striped table_thead_fixed" id="tab_Test3" >
									<thead>
										<tr class="tabTh">
											...
											<td width="75%"><?php echo L('CATEGORY_NAME');?></td>
											<td width="20%"><?php echo L('OPERATING');?></th>
										</tr>
									</thead>
									<tbody>
										<?php if(is_array($category_list)): $i = 0; $__LIST__ = $category_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
												<td style="text-align:center">
													<div class="checkbox checkbox-primary">
														<input name="category_list[]" class="check_list" type="checkbox" value="<?php echo ($vo["category_id"]); ?>"/>
														<label for=""></label>
													</div>
												</td>
												<td><?php echo ($vo["name"]); ?></td>
												<td><a class="edit_category" rel="<?php echo ($vo["category_id"]); ?>" href="javascript:void(0)"><?php echo L('EDIT');?></a> &nbsp;
												<a href="<?php echo U('knowledge/index','category_id='.$vo['category_id']);?>"><?php echo L('VIEW_THIS_CATEGORY');?></a></td>
		...
		if(id_array.length == 0){
			...
			function(isConfirm){
	        	if (isConfirm) {
					$.ajax({
						type: "POST",
						url: "<?php echo U('knowledge/categorydelete');?>",
			...
			//$("#form1").attr('action', '<?php echo U("knowledge/categoryDelete");?>');
			//$("#form1").submit();
		}
	}
</script>
