				...
				<div class="title-bar" style="position: relative;z-index: 99;">
					...
					<div class="row " id="title-show">
						<ul class="nav pull-left" style="margin:2px 0 0 15px;">
							<a class="btn btn-primary btn-sm" href="<?php echo U('announcement/add');?>"><i class="fa fa-plus"></i>&nbsp; <?php echo L('ADD THE ANNOUNCEMENT');?></a>
						</ul>
						...
						<ul class="breadcrum pull-right" style="margin-bottom: 0px">
							<li>
								...
								<div class="input-group">
									...
									<input id="short_search" type="text" style="width:160px;" placeholder="请输入公告标题" onkeydown='if(event.keyCode==13) {$("#short_search_btn").trigger("click");return false;}' class="form-control input-sm" name="search" <?php if($_GET['field'] == 'name'): ?>value="<?php echo ($_GET['search']); ?>"<?php endif; ?>/>
								...
								<?php else: ?>
									<table class="table table-hover table-striped table_thead_fixed" id="tab_Test3" style="background:#fff;margin-bottom:0px;">
										<tr id="childNodes_num" class="tabTh">
											...
											<td><?php echo L('HEADLINE');?></td>
											<td><?php echo L('CREATOR_ROLE');?></td>
											<td>
												<?php if($_GET['asc_order'] == 'update_time'): ?><a href="<?php echo U('announcement/index','desc_order=update_time&'.$parameter);?>"><?php echo L('UPDATE TIME');?>&nbsp;<img src="__PUBLIC__/img/arrow_up.png"></a>
												<?php elseif($_GET['desc_order'] == 'update_time'): ?>
													<a href="<?php echo U('announcement/index','asc_order=update_time&'.$parameter);?>"><?php echo L('UPDATE TIME');?>&nbsp;<img src="__PUBLIC__/img/arrow_down.png"></a>
												<?php else: ?>
													<a href="<?php echo U('announcement/index','desc_order=update_time&'.$parameter);?>"><?php echo L('UPDATE TIME');?></a><?php endif; ?>
											</td>
											<td><?php echo L('THE CURRENT STATE');?></td>
										</tr>
										<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="controls_tr">
												<td>
													<div class="checkbox checkbox-primary">
														<input name="announcement_id[]" class="check_list" type="checkbox" value="<?php echo ($vo["announcement_id"]); ?>"/>
														<label for=""></label>
													</div>
												</td>
												...
												<td><notempty name="vo.update_time"><?php echo (date("Y-m-d",$vo["update_time"])); ?><notempty></td>
												<td>
													<?php if($vo['status'] == '1'): ?><span class="fa fa-circle" style="color:#F5715F"></span>&nbsp;&nbsp;&nbsp;已发布
													<?php elseif($vo['status'] == '2'): ?>
														<span class="fa fa-circle" style="color:#7CCA4E"></span>&nbsp;&nbsp;&nbsp;未发布<?php endif; ?> 
												</td>
								...
								<div id="tfoot_div" class="clearfix">
									<tr>
										<div class="clearfix" id="tfoot_page">	
											<?php if($fields_search): ?><span class="pull-left" style="margin-left:25px;margin-top:10px;">本次搜索结果<span style="color:#F8AC59"> <?php echo ($count); ?> </span>条数据<a href="<?php echo U('announcement/index');?>" class="btn" style="background:#fff;border:1px solid #ccc;margin-left:10px;color:#999;" id="clearnumber">清除搜索条件</a></span><?php endif; ?>
					...
					<ul class="nav pull-left" style="margin:0px 0 0 5px;width:620px;">
						<li class="pull-left">
							<select id="field_<?php echo ($key1); ?>" style="width:auto" class="form-control input-sm field_name new-select" onchange="changeCondition(<?php echo ($key1); ?>)" >
								<option class="" value="name">--请选择条件--</option>
								<option class="word" <?php if($vo['field'] == 'title'): ?>selected="selected"<?php endif; ?> value="title"><?php echo L('HEADLINE');?></option>
								...
								<option class="date" <?php if($vo['field'] == 'create_time'): ?>selected="selected"<?php endif; ?> value="create_time"><?php echo L('CREATE_TIME');?></option>
								<option class="date" <?php if($vo['field'] == 'update_time'): ?>selected="selected"<?php endif; ?> value="update_time">更新时间</option>
							</select>&nbsp;&nbsp;
						</li>
						...
						<?php if($vo["form_type"] != 'box' && $vo["form_type"] != 'address'): if($vo["form_type"] == 'datetime'): ?><input id="start_<?php echo ($vo['field']); ?>" type="text" class="form-control input-sm search-query" name="<?php echo ($vo['field']); ?>[start]" onclick="WdatePicker()" value="<?php echo ($fields_search[$vo['field']][start]); ?>" rel="announcement"/> 至 <input id="end_<?php echo ($vo['field']); ?>" type="text" class="form-control input-sm search-query" name="<?php echo ($vo['field']); ?>[end]" onclick="WdatePicker()" value="<?php echo ($fields_search[$vo['field']][end]); ?>" rel="announcement"/>
...
<div class="hide" id="dialog-role-info" title="<?php echo L('DIALOG_USER_INFO');?>">
...
<div class="" style="display:none;" id="dialog-import" title="<?php echo L('IMPORT DATA');?>">
	<div class="spiner-example">
		<div class="sk-spinner sk-spinner-three-bounce">
			<div class="sk-bounce1"></div>
			<div class="sk-bounce2"></div>
...
$("#return_check").click(function(){
	window.location.href="<?php echo U('contract/revokeCheck', 'id=');?>"+$(this).attr('rel');
});
$("#edit_contract").click(function(){
	window.location.href="<?php echo U('announcement/edit','id=');?>"+$(this).attr('rel');
});
	...
	$('#delete').click(function(){
		...
		function(isConfirm){
	        if (isConfirm) {
	        	$.ajax({
		            type:'post',
		            url: "<?php echo U('announcement/delete');?>",
