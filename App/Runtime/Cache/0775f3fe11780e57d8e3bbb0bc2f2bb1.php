				...
				<div class="title-bar" style="position: relative;z-index: 99;">
					<div class="row  clearfix" id="title-hide" style="display:none;">
						<ul class="breadcrum pull-left">
							...
							<li><a id="delete" href="javascript:void(0)"><i class="fa fa-times-circle"></i>&nbsp;<?php echo L('DELETE');?></a></li>
					...
					<div class="row " id="title-show">
						<ul class="nav pull-left" style="margin:2px 0 0 15px;">
							<a href="<?php echo U('knowledge/add','category_id='.$_GET['category_id']);?>" class="btn btn-primary btn-sm pull-left" style="margin-right:8px"><i class="fa fa-plus-circle"></i>&nbsp; <?php echo L('ADD_KNOWLEDGE');?></a>
						</ul>
						...
						<ul class="breadcrum pull-right" style="margin-bottom: 0px">
							<li>
								...
								<div class="input-group">
									...
									<input id="short_search" type="text" style="width:160px;" placeholder="请输入知识标题" onkeydown='if(event.keyCode==13) {$("#short_search_btn").trigger("click");return false;}' class="form-control input-sm" name="search" <?php if($_GET['field'] == 'title'): ?>value="<?php echo ($_GET['search']); ?>"<?php endif; ?>/>
							...
							<div class="btn-group ">
                                ...
                                <ul class="dropdown-menu">
                                	<?php if(checkPerByAction('knowledge','excelimport')): ?><li><a id="import_excel" class="link" href="javascript:void(0);"><i class="fa fa-upload"></i>&nbsp;导入</a></li><?php endif; ?>
                                    <?php if(checkPerByAction('knowledge','excelexport')): ?><li><a href="javascript:void(0);" class="excelExport link" ><i class="fa fa-download"></i>&nbsp;导出</a></li><?php endif; ?>
									<li><a href="<?php echo U('knowledge/category');?>" id="" class="link"><i class="fa fa-asterisk"></i>&nbsp;分类设置</a></li>
                                </ul>
                            </div>
						</ul>
						</form>
	...
	<img src="__PUBLIC__/img/exclamation_mark.png" style="margin-top:-3px;">&nbsp;&nbsp;暂无数据
										...
										<?php else: ?>
											<tr id="childNodes_num" class="tabTh">
												...
												<td><?php echo L('AUTHORS');?></td>
												<?php if(C('ismobile') != 1): ?><td><?php echo L('CATEGORY');?></td><?php endif; ?>
												<td>
											...
											<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="controls_tr">
													<td style="width:6px;padding-left: 20px">
														<div class="checkbox checkbox-primary">
															<input name="knowledge_id[]" class="check_list" type="checkbox" value="<?php echo ($vo["knowledge_id"]); ?>"/>
															<label for=""></label>
														</div>
													</td>
									...
									<div id="tfoot_div" class="clearfix">
										<tr>
											<div class="clearfix" id="tfoot_page">	
												<?php if($fields_search): ?><span class="pull-left" style="margin-left:25px;margin-top:10px;">本次搜索结果<span style="color:#F8AC59"> <?php echo ($count); ?> </span>条数据<a href="<?php echo U('knowledge/index');?>" class="btn" style="background:#fff;border:1px solid #ccc;margin-left:10px;color:#999;" id="clearnumber">清除搜索条件</a></span><?php endif; ?>
					...
					<ul class="nav pull-left" style="margin:0px 0 0 5px;width:620px;">
						<li class="pull-left">
							<select id="field_<?php echo ($key1); ?>" style="width:auto" class="form-control input-sm field_name new-select" onchange="changeCondition(<?php echo ($key1); ?>)" >
								<option class="" value="name">--<?php echo L('PLEASE_SELECT_A_FILTER_CONDITION');?>--</option>
								<option class="word" <?php if($vo['field'] == 'title'): ?>selected="selected"<?php endif; ?> value="title"><?php echo L('TITLE');?></option>
								<option class="word" <?php if($vo['field'] == 'content'): ?>selected="selected"<?php endif; ?> value="content"><?php echo L('CONTENT');?></option>
								<option class="role" <?php if($vo['field'] == 'role_id'): ?>selected="selected"<?php endif; ?> value="role_id"><?php echo L('AUTHORS');?></option>
								<option class="date" <?php if($vo['field'] == 'create_time'): ?>selected="selected"<?php endif; ?> value="create_time"><?php echo L('CREATOR_TIME');?></option>
								<option class="date" <?php if($vo['field'] == 'update_time'): ?>selected="selected"<?php endif; ?> value="update_time"><?php echo L('UPDATE_TIME');?></option>
								<option class="number" <?php if($vo['field'] == 'hits'): ?>selected="selected"<?php endif; ?> value="hits"><?php echo L('CLICK_NUM');?></option>
							</select>&nbsp;&nbsp;
						</li>
						...
						<?php elseif($vo["field"] == 'role_id' || $vo["form_type"] == 'datetime'): ?>
						<?php else: ?>
						...
						<?php if($vo["form_type"] != 'box' && $vo["form_type"] != 'address'): if($vo["form_type"] == 'datetime'): ?><input id="start_<?php echo ($vo['field']); ?>" type="text" class="form-control input-sm search-query" name="<?php echo ($vo['field']); ?>[start]" onclick="WdatePicker()" value="<?php echo ($fields_search[$vo['field']][start]); ?>" rel="knowledge"/> 至 <input id="end_<?php echo ($vo['field']); ?>" type="text" class="form-control input-sm search-query" name="<?php echo ($vo['field']); ?>[end]" onclick="WdatePicker()" value="<?php echo ($fields_search[$vo['field']][end]); ?>" rel="knowledge"/>
							<?php elseif($vo['field'] == 'role_id'): ?>
							...
							<?php else: ?>	
								<input name="<?php echo ($vo['field']); ?>[value]" type="text" class="form-control input-sm" class="<?php echo ($vo['form_type']); ?>" value="<?php echo ($_GET[$vo['field']][value]); ?>" rel="knowledge"><?php endif; endif; ?>
						</li>
					</ul>
				</div>
...
$(document).ready(function(){
   $('#knowledge-tree').jstree({
    ...
    $("#edit_knowledge").click(function(){
		window.location.href="<?php echo U('knowledge/edit', 'id=');?>"+$(this).attr('rel');
	})
...
$('#delete').click(function(){
	...
	function(isConfirm){
        if (isConfirm) {
        	$.ajax({
	            type:'post',
	            url: "<?php echo U('knowledge/delete');?>",
...
$(function(){
	$(".excelExport").click(function(){
		if(count > limit_size){
			...
			swal({   
				title: "<?php echo L('EXPORT_THE_KNOWLEDGE');?>",
				type: "warning",   
				showCancelButton: true,   
				confirmButtonColor: "#DD6B55",   
				confirmButtonText: "确定",   
