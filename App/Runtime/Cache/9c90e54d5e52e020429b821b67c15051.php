...
<script>
	...
	<form class="form-horizontal" id="form" role="form" action="<?php echo U('business/add');?>" method="post" enctype="multipart/form-data">
							...
							<div class="col-md-11 add_body_form" style="margin:20px 0 0 20px;">
								<div class="form-group">
									...
									<div class="col-md-6">
										<div class="input-daterange input-group">
											<span class="input-group-addon" style="background-color:#ccc;"><?php echo ($business['business_custom']); ?></span>
											<input type="text" name="code" rel="require" class="form-control" value="<?php echo ($business['code']); ?>"/>
											<input type="hidden" id="prefixion" name="prefixion" value="<?php echo ($business['
											business_custom']); ?>"/>
										</div>
									</div>
								...
								<?php if(is_array($field_list['main'])): $key = 0; $__LIST__ = $field_list['main'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key; if($vo['field'] == 'status_id'): ?><div class="form-group">
										...
										<div class="form-group">
											...
											<div class="col-md-6">
												<input type="hidden" name="customer_id" id="customer_id" value="<?php echo ($business["customer_id"]); ?>" />
												<input class="form-control required" aria-required="true" placeholder="请点击选择"  type="text" name="customer_name" id="customer_name" value="<?php echo ($business["customer_name"]); ?>" readonly="true" style="cursor:pointer;" title="请点击选择"/>
											</div>
										...
										<div class="form-group">
											...
											<div class="col-md-6">
												<input type="hidden" name="contacts_id" id="contacts_id" value="<?php echo ($business["contacts_id"]); ?>" />
												<input class="form-control " aria-required="true" placeholder="请点击选择" type="text" name="contacts_name" value="<?php echo ($business["contacts_name"]); ?>" id="contacts_name" readonly="true" style="cursor:pointer;" title="请点击选择"/>
											</div>
										...
										<div class="form-group">
											...
											<div class="col-md-6">
												<input type="hidden" name="joiner" id="user_id" value="" />
												<input class="form-control required" aria-required="true" placeholder="请点击选择"  type="text" name="joiner_name" id="user_name" value="" readonly="true" style="cursor:pointer;" title="请点击选择"/>
											</div>
											<div class="col-md-2"><span style="color:red;line-height: 32px;margin-left: 10px;">*</span></div>
										</div>
											...
											<?php if($vo['form_type'] == 'textarea'): if($vo['tip_start'] == 1): ?><div class="col-md-6">
												...
												<div class="col-md-6">
													<select name="possibility" id="possibility" class="form-control">
														<?php if(is_array($possibility_list)): $key1 = 0; $__LIST__ = $possibility_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key1 % 2 );++$key1;?><option value="<?php echo ($vo); ?>%"><?php echo ($vo); ?>%</option><?php endforeach; endif; else: echo "" ;endif; ?>
													</select>
												</div>
												<div class="col-md-2"></div>
												<?php elseif($vo['field'] == 'name'): ?>
												<div class="col-md-6">
													<input class="form-control required" id="name" name="name" placeholder="" type="text" value="<?php echo ($business['name']); ?>" />
												</div>
												<?php if($vo['tip_start'] == 1): ?><div class="col-md-2"><span style="color: red;line-height: 32px;margin-left: 10px;">*</span></div>
													<?php else: ?>
													<div class="col-md-2"></div><?php endif; ?>
...
<script type="text/javascript">
    var now_rows = <?php echo ($max_key); ?>?<?php echo ($max_key); ?>:0;
    var total_row_id = <?php echo ($max_key); ?>?<?php echo ($max_key); ?>:0;
    //减少一条信息
...
<script type="text/javascript">
    ...
    //删除 函数
    function del_obj(id){
        $.post("<?php echo U('file/filedel');?>",{file_id:id},function(data){
            if(data.status == 1){
                $('#del_'+id).remove();
