					...
					<div class="col-lg-12">
						...
						<div style="padding:10x;margin:15px 20px;">
							<div class="radio radio-info radio-inline">
								<input type="radio" name="openrecycle" <?php if($examineinfo['contract_examine'] == 1): ?>checked<?php endif; ?> id="openrecycle1" class="openrecycle" value="2">
								<label for="openrecycle1">设置审批流程</label>&nbsp;
								<span style="color:#ccc;">&nbsp;(&nbsp;启用后,审批流程将会固定，用户无需选择下一审批人&nbsp;)</span>
							</div>
							<div id="status_a" <?php if($examineinfo['contract_examine'] == 0): ?>class="status_a"<?php endif; ?> style="margin-top:10px;">
								<div style="border:1px solid #ddd;padding:15px;" id="content_form">
									<div class="pull-left">
										<?php if($examine_list): if(is_array($examine_list)): $i = 0; $__LIST__ = $examine_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($key > 0): ?><div class="pull-left" style="margin-top:12px;margin-left:15px;font-size:22px;color:#ccc;"><i class="fa fa-long-arrow-right"></i></div><?php endif; ?>
												<span class="pull-left">
													<div style="clear:both;">
														<?php if($vo['user_info']['thumb_path']): ?><img class="img-circle" style="width:50px;height:50px;border-radius:50%;margin-left:15px;" src="<?php echo ($vo['user_info']['thumb_path']); ?>"/>
														<?php else: ?>
															<img class="img-circle" style="width:50px;height:50px;border-radius:50%;margin-left:15px;" src="__PUBLIC__/img/avatar_default.png"/><?php endif; ?>
													</div>
													<div style="clear:both;text-align:center;margin-left:15px;margin-top:5px;">
														<?php echo ($vo['user_info']['full_name']); ?>
													</div>
												</span><?php endforeach; endif; else: echo "" ;endif; ?>
										<?php else: ?>
											<span style="vertical-align:middle;color:#ccc;">您还没有设置审批人！</span><?php endif; ?>
									</div>
									<button type="button" class="btn btn-danger pull-right" id="cleartype" <?php if($examineinfo['contract_examine'] == 0): ?>disabled="disabled"<?php endif; ?> >清空</button>
									<button type="button" id="edit_process" rel="<?php echo ($status_id); ?>" class="btn btn-primary pull-right" style="margin-right:5px;" <?php if($examineinfo['contract_examine'] == 0): ?>disabled="disabled"<?php endif; ?>>
										<?php if($examine_list): ?>编辑<?php else: ?>新增<?php endif; ?>
									</button>
							...
							<div style="margin-top:15px;">
								<div class="radio radio-info radio-inline">
									<input type="radio" name="openrecycle" <?php if($examineinfo['contract_examine'] == 0): ?>checked<?php endif; ?> id="openrecycle" class="openrecycle" value="1" />
...
$('#cleartype').click(function(){
	...
	function(isConfirm){
		if (isConfirm) {
			$.ajax({
		        type:'post',
		        url: "<?php echo U('contract/clearexamine');?>",
...
$("#dialog-edit-process").dialog({
	...
	buttons: {
		"保存": function () {
			window.location.href = '<?php echo U("contract/examine");?>';
			$(this).dialog("close");
		},
		"取消": function () {
			$(this).dialog("close");
...
$('#edit_process').click(function(){
	...
	$('#dialog-edit-process').load("<?php echo U('contract/examinedata');?>");
});
...
$('.openrecycle').change(function(){
	...
	if(openrecycle){
		$.ajax({
			type: "post",
			async:false,
			url: "<?php echo U('contract/examinedata');?>", 
