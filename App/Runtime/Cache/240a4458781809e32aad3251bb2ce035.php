				...
				<div class="title-bar">
					<div class="row " id="title-show">
						<div class="pull-left breadcrum">
							<a href="javascript:void(0);" id="add_event" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> &nbsp;<?php echo L('NEW_SCHEDULE');?></a>&nbsp;
							<!-- <div class="btn-group">
								<button class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown"><i class="fa fa-wrench"></i>  &nbsp;<?php echo L('SCHEDULING_TOOLS');?><span class="caret"></span></button>
								<ul class="dropdown-menu">
									<li><a href="javascript:void(0);" id="import_excel"  class="link">导入日程</a></li>
									<li><a href="javascript:void(0);" id="excelExport" class="link"><i class="fa fa-download"></i>&nbsp;<?php echo L('EXPORT_THE_SCHEDULE');?></a></li>
								</ul>
							</div> -->
						</div>
					</div>
...
$(document).ready(function() {
    ...
    $('#calendar').fullCalendar({
    	// events: <?php echo ($eventlist); ?>,
    	events: "<?php echo U('event/indexdata');?>",
        header: {
        ...
        dayClick: function(date, allDay, jsEvent, view) {
			...
			var url = "<?php echo U('event/add','start_date=');?>"+start_date+'&end_date='+end_date;
			$('#add_modal').load(url);
    	},
    	...
    	eventClick: function(calEvent, jsEvent, view) {
			...
			var url = "<?php echo U('event/view','event_id=');?>"+event_id+'&module='+module+'&module_id='+module_id;
			$('#view_event').load(url);
    	}
    });
});
