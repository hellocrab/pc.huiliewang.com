...
<div class="clearfix">
	<form class="form-inline" id="dd" action="<?php echo U('user/listdialog');?>">
			...
			<?php if(role_list != null): if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr style="cursor:pointer">
						<td>
							<div class="checkbox checkbox-info checkbox-inline" style="margin-left:15px;">
								<input name="owner" class="owner" type="checkbox" rel="<?php echo ($vo['user_name']); ?>" rel2="<?php echo ($vo['img']); ?>" value="<?php echo ($vo["role_id"]); ?>"><label for=""></label>
							</div>
						</td>
