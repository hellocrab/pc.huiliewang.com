...
<script>
	...
	<form class="form-horizontal" id="form" role="form" action="<?php echo U('contract/add');?>" method="post" enctype="multipart/form-data">
		<input type="hidden" name="refer_url" value="<?php echo ($refer_url); ?>" />
		<input type="hidden" name="total_amount_val" id="total_amount" value="0" />
		<input type="hidden" name="renew_contract_id" value="<?php echo ($old_contract_id); ?>">
		<input type="hidden" name="renew_parent_id" value="<?php echo ($renew_parent_id); ?>">
					...
					<div class="full-height-scroll">
						<div class="row">
							...
							<div class="col-md-11 add_body_form" style="margin:20px 0 0 20px;">
								<?php if(is_array($field_list['main'])): $key = 0; $__LIST__ = $field_list['main'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key; if($vo['field'] == 'customer_id'): ?><div class="form-group">
											<label class="col-md-4 control-label"><?php echo L('SOURCE_OF_CUSTOMERS');?>：</label>
											<div class="col-md-6">
												<input type="hidden" name="customer_id" id="customer_id" value="<?php echo ($contract_info["customer_id"]); ?>" />
												<input class="form-control required" aria-required="true" placeholder="请点击选择" type="text" name="customer_name" id="customer_name" value="<?php echo ($contract_info["customer_name"]); ?>" readonly="true" style="cursor:pointer;" title="请点击选择"/>
											</div>
											<div class="col-md-2"><span style="color:red">*</span></div>
										</div>
									<?php elseif($vo['field'] == 'business_id'): ?>
										<?php if($old_contract_id): ?><div class="form-group" style="display: none">
										...
										<?php else: ?>
											<div class="form-group" style="display: none">
												<label class="col-md-4 control-label">来源项目：</label>
												<div class="col-md-6">
													<input type="hidden" name="business_id" id="business_id" value="<?php echo ($contract_info["business_id"]); ?>"/>
													<input class="form-control" placeholder="请点击选择" type="text" name="business_name" id="business_name" value="<?php echo ($contract_info["business_name"]); ?>" readonly="true" style="cursor:pointer;" title="请点击选择"/>
												</div>
												<div class="col-md-2"></div>
											</div><?php endif; ?>
									<?php elseif($vo['field'] == 'contract_name'): ?>
										<div class="form-group">
											<label class="col-md-4 control-label"><?php echo L('CONTRACT_NO');?>：</label>
											<div class="col-md-6">
												<div class="input-daterange input-group">
													<span class="input-group-addon" style="background-color:#ccc;"><?php echo ($contract_custom); ?></span>
													<input type="text" id="number" name="number" rel="require" class="form-control checkit" value="<?php echo ($number); ?>"/>
													<input type="hidden" id="prefixion" name="prefixion" value="<?php echo ($contract_custom); ?>"/>
												</div>
											</div>
											<div class="col-md-2"><span style="color:red">*</span></div>
										</div>
										<div class="form-group">
											...
											<div class="col-md-6">
												<input class="form-control required" aria-required="true" type="text" name="contract_name" value="<?php if($contract_info[contract_name]): ?>续约《<?php echo ($contract_info['contract_name']); ?>》<?php endif; ?>" id="contract_name" />
											</div>
											<div class="col-md-2"><span style="color:red">*</span></div>
										</div>
										<div class="form-group">
											...
											<div class="col-md-6">
												...
												<input class="form-control required" aria-required="true" type="text" name="owner_role_name" id="owner_name" value="<?php echo (session('full_name')); ?>" readonly="true" style="cursor:pointer;" title="请点击选择"/>
											</div>
											<div class="col-md-2"><span style="color:red">*</span></div>
										</div>
									<?php elseif($vo['field'] == 'price'): ?>
										<div class="form-group">
											<label class="col-md-4 control-label">合同金额(元)：</label>
											<div class="col-md-6">
												<input class="form-control required" type="text" id="contract_price" name="price" value="<?php echo ($contract_info['final_price']); ?>" placeholder="如：20000" onblur="bu(this)" onkeyup="final_price_sum('contract_price');"/>
											</div>
											<div class="col-md-2"><span style="color:red">*</span></div>
										</div>
									<?php elseif($vo['field'] == 'start_date'): ?>
										<div class="form-group">
											<label class="col-md-4 control-label"><?php echo L('THE_TIME_WHICH_THE_CONTRACT_COMES_INTO_FORCE');?>：</label>
											<div class="col-md-6">
									...
									<?php elseif($vo['field'] == 'end_date'): ?>
										<div class="form-group">
											<label class="col-md-4 control-label"><?php echo L('CONTRACT_TIME');?>：</label>
											<div class="col-md-6">
							...
							<?php if($field_list['data']): ?><div class="col-md-12 add_body_title">
									<div class="all-inline">
										<span class="sq-tag"></span>
										<div class="text-tag">
											<span>附加信息</span>
						...
						<div class="col-md-12 ibox-content" style="border:0px;">
							<div class="panel-body" style="padding:0px;">
								<table class="table table-bordered" id="no-input-border" width="95%" border="0" cellspacing="1" cellpadding="0">
									...
									<?php $max_key = 1;?>
									<?php if($contract_info['business_product_list']): ?><tbody>
											<?php if(is_array($contract_info['business_product_list'])): $i = 0; $__LIST__ = $contract_info['business_product_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="row_<?php echo ($key+1); ?>">
													<td style="text-align:center;">
														<a href="javascript:void(0);" class="reduce_one"><i class="icon-minus"></i></a>
													</td>
													<td>
														<input type="hidden" name="business[product][<?php echo ($key+1); ?>][product_id]" id="product_id_<?php echo ($key+1); ?>" class="cproduct_id" value="<?php echo ($vo["product_id"]); ?>"/>
														<input type="text" class="form-control" id="product_name_<?php echo ($key+1); ?>" style="border:none;!important" readonly="true" value="<?php echo ($vo["product_name"]); ?>"/>
													</td>
													<td><!-- 原价 -->
														<input type="text" id="product_ori_price_<?php echo ($key+1); ?>" name="business[product][<?php echo ($key+1); ?>][ori_price]" class="form-control" style="border:none;!important" readonly="readonly"  value="<?php echo ($vo["ori_price"]); ?>" />
													</td>
													<td><!-- 折率 -->
														<input type="text" name="business[product][<?php echo ($key+1); ?>][discount_rate]" id="product_discount_rate_<?php echo ($key+1); ?>" style="" class="form-control" onkeyup="calculate(<?php echo ($key+1); ?>,1)" <?php if($vo['discount_rate'] == 0): ?>value="0.00"<?php else: ?>value="<?php echo ($vo["discount_rate"]); ?>"<?php endif; ?>/>
													</td>
													<td>
														<input type="text" class="form-control" name="business[product][<?php echo ($key+1); ?>][unit_price]" id="product_unit_price_<?php echo ($key+1); ?>" onkeyup="calculate(<?php echo ($key+1); ?>)" style="" value="<?php echo ($vo["unit_price"]); ?>" />
													</td>
													<td><!-- 商品数量 -->
														<input type="text" class="form-control amount" name="business[product][<?php echo ($key+1); ?>][amount]" style="" id="product_amount_<?php echo ($key+1); ?>" onkeyup="calculate(<?php echo ($key+1); ?>)" value="<?php echo ($vo["amount"]); ?>"/>
													</td>
													<td>
														<input type="text" style="border:none;!important" name="business[product][<?php echo ($key+1); ?>][unit]" readonly="readonly" class="form-control" value="<?php echo ($vo["unit"]); ?>"/>
													</td>
													<td><!-- 小计 -->
														<input type="text" style="border:none;!important" name="business[product][<?php echo ($key+1); ?>][subtotal]" id="product_subtotal_<?php echo ($key+1); ?>" class="form-control subtotal" value="<?php echo ($vo["subtotal"]); ?>" readonly="readonly" rel="<?php echo ($key+1); ?>"/>
													</td>
												</tr>
								...
								<div class="ibox" style="overflow-x: hidden;margin: 10px 5px 0px;">
									<div class="row" style="margin: 10px 5px 0px;">
										<div class="col-md-4" style="padding-top:9px">
											产品合计(元):&nbsp;
											<span id="total_subtotal_val"><?php echo ($contract_info['sales_info']['prime_price']); ?></span>
											<input type="hidden" value="<?php echo ($contract_info['sales_info']['prime_price']); ?>" id="prime_price" name="prime_price">
										</div>
										<div class="col-md-4">整单折扣(%):&nbsp;
											<input style="display: inline-block; width: 60px; padding: 1px;" type="text" id="final_discount_rate" name="final_discount_rate" class="form-control" onkeyup="final_price_sum('rate');" value="<?php echo ($contract_info['sales_info']['final_discount_rate']); ?>" />
										</div>
										<div class="col-md-4">合同金额(元):&nbsp;
											<input style="display: inline-block; width: 100px; padding: 1px;" type="text" name="final_price" id="final_price" class="form-control" placeholder="0.00" onkeyup="final_price_sum('price');" value="<?php echo ($contract_info['final_price']); ?>" onblur="bu(this)" />
											<input type="hidden" id="total_amount_val" value="<?php echo ($contract_info['total_amount']); ?>" name="total_amount">
										</div>
									</div>
								</div>
							</div>
...
<div style="display:none;"  id="dialog-customer" title="<?php echo L('SELECT_BUSINESS_OPPORTUNITIES');?>">
...
<div style="display:none;"  id="dialog-customer-list" title="<?php echo L('SELECT_CUSTOMER');?>">
	<div class="spiner-example">
		<div class="sk-spinner sk-spinner-three-bounce">
			<div class="sk-bounce1"></div>
			<div class="sk-bounce2"></div>
...
<div style="display:none;"  id="dialog-role-list2" title="<?php echo L('SELECT_HEAD');?>">
	<div class="spiner-example">
		<div class="sk-spinner sk-spinner-three-bounce">
			<div class="sk-bounce1"></div>
			<div class="sk-bounce2"></div>
...
<script type="text/javascript">
	$('#number').blur(function(){
		...
		$.ajax({
			type: "post",
			async:false,
			url: "<?php echo U('contract/yanchong');?>",
	...
	$(function(){
		$(document).on('click','#business_name',function () {
			...
			$('#dialog-customer').load('<?php echo U("customer/clistDialog","customer_id=");?>'+customer_id);
		});
...
<script type="text/javascript">
	...
	var total_row_id = <?php echo ($max_key); ?>;
	//减少一条信息
...
$(".addfile").click(function(){
	...
	$('#dialog-file').load('<?php echo U("file/addlogfile");?>');
});
