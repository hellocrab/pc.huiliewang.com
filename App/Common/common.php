...
//        $departmentIds .= ','.$id;
        array_push($list,$id);
        if(!$sonInfo = M('roleDepartment')->where('parent_id =%d', $id)->find()){
            continue;
        }
        if($id <= 0){
            continue;
        }
        getSubDepartmentBrId($sonInfo['department_id']);
    }
    $departmentIds = implode(',',$list);
...
function sendMessage($id, $content, $sysMessage = 0, $send_type) {
    ...
    $m_config = M('Config');
    if ($sysMessage == 0)
        $data['from_role_id'] = session('role_id');
    $data['to_role_id'] = $id;
    if ($send_type == 2) {
...
function sysSendEmail($to_role_id, $title, $content, $author) {
    ...
    if (!$to_role_id)
        return false;
    if (!$author)
        $author = C('defaultinfo.name') . L('ADMIN');
    import('@.ORG.Mail');
    $to_user = D('RoleView')->where('role.role_id = %d', $to_role_id)->find();
    if (!is_email($to_user['email']))
        return false;
...
function userSendEmail($address, $title, $content, $author = false) {
    ...
    if ($author === true)
        $author = C('defaultinfo.name') . '-' . $user['name'];
    else
        $author = C('defaultinfo.name');
    import('@.ORG.Mail');
    if (!is_email($address))
        return false;
...
function bsendemail($address, $title, $content, $file = array(), $author = false, $selfsmtp = false) {
    ...
    if ($author === true)
        ...
        $author = C('defaultinfo.name') . "-huiliewang";
                    ...
                    if ($setting['type'] == 'select') {
                        ...
                        if ($v['field'] == 'pro_type') {
                            foreach ($setting['data'] as $k2 => $v2) {
                                $str .= "<option value='$k2'";
                                $str .= $value == $k2 ? ' selected="selected" ' : '';
                                $str .= ">$v2</option>";
                            }
                        } else {
                ...
                default:
                    if ($v['field'] == 'create_time' || $v['field'] == 'update_time') {
                        ...
                        if ($v['is_recheck'] == 1) {
                            ...
                            $field_list[$k]['html'] = '<input class="form-control ' . $required . '"  type="text" id="' . $v['field'] . '" name="' . $v['field'] . '" ' . $maxlength . ' value="' . $value . '" placeholder="' . $v['input_tips'] . '" /><span id="' . $v['field'] . 'Tip" style="float: left;line-height: 32px;margin-left: 5%;color:red;"></span>';
//                            $field_list[$k]['html'] =$input_tips;
                        }
                    }
                    break;
            }
...
function sendtestSMS($uid, $uname, $telphone) {
    ...
    $argv = array(
        ...
        'mobile' => $telphone,
        'content' => urlencode('TEST SMS 【huiliewang】'),
        'ext' => '',
        'rrid' => '',
        'stime' => $sendtime
    );
...
function check_permission($module_id, $module, $permission_field = 'owner_role_id') {
    ...
    if (in_array($owner_role_id, $permission_ids) || !$owner_role_id)
        return true;
    else
        return false;
}
