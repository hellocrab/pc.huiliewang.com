	...
	**/
	public function add(){
		...
		if($this->isPost()){
			...
			}elseif (intval($_GET['customer_id'])){
                $contract_info['customer_id'] =$_GET['customer_id'];
                $contract_info['customer_name'] = M('Customer')->where('customer_id = %d',$_GET['customer_id'])->getField('name');
                $this->contract_info = $contract_info;
...
//            var_dump($field_list['main'][1]);exit();
