...
class CustomerAction extends Action {
    ...
    public function _initialize() {
        ...
        $action = array(
            'permission' => array('analytics'),
            'allow' => array('validate', 'check', 'checkinfo', 'getcustomeroriginal', 'close', 'getcurrentstatus', 'excelimportact', 'clistdialog', 'edit_ajax', 'settop', 'yc_share', 'share_list', 'ajax_share', 'analytics')
        );
...
//                dump($this->field_list['main']);die;
    ...
    public function edit() {
        ...
        $user_ids = explode(",", $customer['customer_owner_id']);
        $customer_owner_id = array();
        if ($user_ids) {
            foreach ($user_ids as $list) {
                $user = M("user")->where("role_id=%d", $list)->field("full_name")->find();
                $customer_owner_id[] = $user['full_name'];
            }
            $this->customer_owner_name = implode(",", $customer_owner_id);
        }
        ...
        if (!in_array($customer['owner_role_id'], $this->_permissionRes) && !in_array($customer['transfer_role'], $this->_permissionRes))
        ...
        if ($this->isPost()) {
            ...
            if ($m_customer->create()) {
                if ($m_customer_data->create() !== false) {
                    ...
                    ContactsModel::updateLog($customer['contacts_id'],$customer['customer_id'], $_POST['con_contacts']);
                    //联系人信息修改
                    ContactsModel::contactsUp($customer['contacts_id'],$_POST['con_contacts'],$customer['customer_id']);
    ...
    public function index() {
        ...
        switch ($by) {
            case 'todaycontact' :
                ...
                break;
            case 'today' : $where['create_time'] = array('gt', strtotime(date('Y-m-d', time())));
                break;
            case 'week' : $where['create_time'] = array('gt', (strtotime(date('Y-m-d')) - (date('N', time()) - 1) * 86400));
                break;
            case 'month' : $where['create_time'] = array('gt', strtotime(date('Y-m-01', time())));
                break;
            case 'add' : $order = 'customer.create_time desc,customer.customer_id asc';
                break;
            case 'update' : $order = 'customer.update_time desc,customer.customer_id asc';
                break;
            case 'sub' : $where['owner_role_id'] = array('in', $below_ids);
                break;
            case 'me' :
                $where['owner_role_id|transfer_role'] = intval(session('role_id'));
                break;
...
//                        $where['_string'] = "customer.owner_role_id=0 or (customer.update_time < $outdate and customer.is_locked = 0) or (customer.get_time < $contract_outdays and customer.is_locked = 0)";
        ...
        if (!isset($where['owner_role_id']) && $this->_get('content') !== 'resource') {
            if ($by != 'deleted' && $by != 'share' && $by != 'myshare' && $by != 'all' && $by != 'me') {
                $where['owner_role_id'] = array('in', implode(',', $this->_permissionRes));
            }
        }
        if ($this->_get('content') != 'resource') {
                ...
                if ($field == 'name' && $search) {
                    ...
                    switch ($condition) {
                        ...
                        case "start_with" : $where[$field] = array('like', $search . '%');
                            break;
                        case "not_start_with" : $where[$field] = array('notlike', $search . '%');
                            break;
...
//		var_dump(session('role_id'));exit();
        ...
        //场景名称
        switch ($by) {
            case 'me' : $scene_name = '我的客户';
                break;
            case 'sub' : $scene_name = '下属客户';
                break;
            case 'all' : $scene_name = '全部客户';
                break;
            case 'share' : $scene_name = '共享给我的';
                break;
            case 'myshare' : $scene_name = '我共享的';
                break;
            default : $scene_name = '我的客户';
                break;
        }
        if ($scene_id) {
...
//        $where['_string'] = 'update_time > ' . $outdate . ' OR is_locked = 1';
        ...
        foreach ($customer_list as $k => $v) {
            ...
            $customer_list[$k]['address'] = $city_name[$customer_list[$k]['address']];
            $customer_list[$k]['industry'] = $industry_name[$customer_list[$k]['industry']];
            $contacts = array();
            if (!empty($v['contacts_id'])) {
...
//            }
            ...
            //维护人查询
            if ($customer['customer_owner_id']) {
                $customer_owner_ids = explode(",", $customer['customer_owner_id']);
                $customer_owner_name = "";
                ...
                $customer['customer_owner_name'] = $customer_owner_name;
            }



//            $customer['customer_owner_name'] = implode(",",$customer_owner_name);
...
//			$this->business = $business;
    ...
    public function view_ajax() {
        $this->customer_id = I('id');
        $d_contacts = D('ContactsView');
        $customer = M("customer")->where("customer_id=%d", I('id'))->find();
        ...
        foreach ($business as $key => $list) {
            if ($list['joiner']) {
                ...
                foreach ($joiner as $k => $li) {
                    $user = M("user")->where("role_id=%d", $li)->field("full_name")->find();
                    $arr[$k]['name'] = $user['full_name'];
                    $arr[$k]['id'] = $li;
                }
...
//                $business[$key]['joiner'] = implode(",",$arr);
        ...
        //沟通日志
        $log_ids = $m_r_customer_log->where('customer_id = %d', $customer['customer_id'])->getField('log_id', true);
        ...
        $this->receivingorder_list = $d_invoice->where("invoice.customer_id=%d", I('id'))->order("create_time", "asc")->select();
        ...
        $b_where['customer_id'] = $this->customer_id;
...
//            $contract_list[$c_k]['invoice_price'] = $m_invoice->where(array('contract_id'=>$c_v['contract_id'],'is_checked'=>array('neq',2)))->sum('price');
    ...
    public function getCustomerOriginal() {
        ...
        header("Content-type: text/html; charset=utf-8");
        $m_customer = M('customer');
//		$original = $m_customer->Distinct(true)->field('industry')->getField('industry',true);
