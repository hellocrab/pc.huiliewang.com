...
//        $this->list =  D("ResumeView")->where($where)->select();
    ...
    public function dialogoffer() {
        ...
        $roleIds = I("roleIds", '');
        $role_id = intval(I("id"));
        $role_id > 0 && $where['fine_project_offer.role_id'] = $role_id;
        ($role_id <= 0 && $roleIds) && $where['fine_project_offer.role_id'] = ['in', $roleIds];
...
//        $this->list =  D("ResumeView")->where($where)->select();
    ...
    public function dialogenter() {
        ...
        $roleIds = I("roleIds", '');
        $role_id = intval(I("id"));
        $role_id > 0 && $where['fine_project_enter.role_id'] = $role_id;
        ($role_id <= 0 && $roleIds) && $where['fine_project_enter.role_id'] = ['in', $roleIds];
...
//            $role_ids = array(intval($_GET['role']));
            ...
            $below_ids = [];
            if ($department_id) {
                $allRoles = getRolesByDepart($department_id, true);
