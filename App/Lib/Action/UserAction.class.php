...
class UserAction extends Action {
	 ...
	 *
	**/
	public $user_type = array('0'=>'猎头','1'=>'财务','2'=>'行政','99'=>'其他');
...
//            $where['role.position_id'] = array('in',$position_ids);
	...
	public function mutiListDialog(){
		...
		$pater_id = isset($_GET['pater']) ? $_GET['pater'] : '';
	...
	public function index(){
		...
		foreach($user_list as $k=>$v){
			...
			$type_name = '';
			switch($v['type']){
				case 1 : $type_name = '猎头';break;
				case 2 : $type_name = '财务';break;
				case 3 : $type_name = '行政';break;
	...
	public function department_add(){
		...
		if($this->isPost()){
            ...
            $department_label=$_POST['department_label'];
            if($department_label){

                $labels="";
                foreach ($department_label as $value){
                    $labels .= $value.",";
                }

                $_POST['department_label']=substr($labels,0,-1);
            }else{
                $_POST['department_label']="";
            }
			...
			$department_label=M('department_lable')->where('isdelete=0')->select();
			$department_list = $department->select();
			$this->assign('departmentList', getSubDepartment(0,$department_list,''));
			$this->assign('department_label', $department_label);
			$this->display();
		}
	}
	...
	public function department_edit(){
		if($this->isPost()){
			...
			$department = M('roleDepartment');
            $departments=$department->where('department_id = %d',intval($_GET['id']))->find();
            $departments['department_label']=explode(',', $departments['department_label']);
...
/*插入下级*/
	...
	public function reset_password(){
		if($this->isAjax()){
			...
			//演示站start
			$user_info = M('User')->where('user_id = %d',$user_id)->find();
			if ($user_info['category_id'] == 1) {
				$this->ajaxReturn('','演示站点不能修改管理员密码！',0);
			}
			//演示站end
	...
	public function  department_label(){
        $where['isdelete']='0';
        $department_label_list = M('department_lable')->where($where)->select();
        $this->department_label =$department_label_list;

        $this->display();
    }

    //新增部门标签
    public function  departmentlabeladd(){
        if($this->isPost()){
            //新增标签
            $repeat['name']=$_POST['label_name'];
            $repeat['isdelete']=0;
            if(M('department_lable')->where($repeat)->select()){
                $this->ajaxReturn('',L('部门标签已存在'),2);
            }
            $data['name']=$_POST['label_name'];
            $data['isdelete']='0';
            if(M('department_lable')->add($data)){
                $this->ajaxReturn('',L('添加成功'),1);
            }else{
                $this->ajaxReturn('',L('添加失败'),2);
            }
        }else{
            $this->display();
        }
    ...
    //修改部门标签
    public function  departmentlabeledit(){

        if($this->isPost()){
            $departmentlabelid=$_POST['id'] ? $_POST['id'] : '';
            ...
            if($departmentlabelid){
                //修改操作
                $data['id']=$departmentlabelid;
                $data['name']=$_POST['label_name'];

                if(M('department_lable')->save($data)){
            ...
            $departmentlabelid=$_GET['id'] ? $_GET['id'] : '';
            $where['id']=$departmentlabelid;
            $departmentlabel=M('department_lable')->where($where)->select();

            if($departmentlabel){
                $this->department_labelid =$departmentlabelid;
                $this->department_labelname =$departmentlabel[0]['name'];
            }else{
                $this->department_labelid ='0';
                $this->department_labelname ='';
            }
    ...
    //删除部门标签
    public function  deldepartmentlabel(){
        $departmentlabelid=$_GET['id'] ? $_GET['id'] : '';
    ...
    //职级管理
    public function jobrank(){
        $where['isdelete']='0';
        $department_label_list = M('job_rank')->where($where)->select();
        $this->department_label =$department_label_list;

        $this->display();
    }

    public  function jobrankadd(){
        if($this->isPost()){

            $repeat['name']=$_POST['jobrank_name'];
            $repeat['isdelete']=0;
            if(M('job_rank')->where($repeat)->select()){
                $this->ajaxReturn('',L('职级已存在'),2);
            }

            //新增职级
            $data['name']=$_POST['jobrank_name'];
            $data['isdelete']='0';
            if(M('job_rank')->add($data)){
                $this->ajaxReturn('',L('添加成功'),1);
            }else{
                $this->ajaxReturn('',L('添加失败'),2);
            }
    ...
    public function  jobrankedit(){
        ...
        if($this->isPost()){
            ...
            $jobrankid=$_GET['id'] ? $_GET['id'] : '';
            $where['id']=$jobrankid;
            $jobrank=M('job_rank')->where($where)->select();
