...
class FileAction extends Action {
    ...
    public function _initialize() {
        $action = array(
            'permission' => array(),
            'allow' => array('editor', 'add', 'delete', 'filedownload', 'add_file', 'addlogfile', 'getfiles', 'filedel')
        );
    ...
    public function filedel() {
        if ($this->isAjax()) {
            ...
            if ($file_id) {
                ...
                switch ($r_module) {
                    case 'examine' : $r = M('ExamineFile');
                        break;
                    case 'business' : $r = M('RBusinessFile');
                        break;
                    case 'contract' : $r = M('RContractFile');
                        break;
                    case 'customer' : $r = M('RCustomerFile');
                        break;
                    case 'finance' : $r = M('RFileFinance');
                        break;
                    case 'leads' : $r = M('RFileLeads');
                        break;
                    case 'log' : $r = M('RFileLog');
                        break;
                    case 'product' : $r = M('RFileProduct');
                        break;
                    case 'member' : $r = M('RMemberFile');
                        break;
                    case 'task' : $r = M('RTaskFile');
                        break;
                }
                if ($r) {
    ...
    public function add() {
        ...
        } elseif ($_GET['r'] && $_GET['module'] && $_GET['RResumeFile']) {
            $defaultinfo = $m_config->where('name = "defaultinfo"')->find();
            $value = unserialize($defaultinfo['value']);
