...
class FinanceAction extends Action
{
    ...
    public function _initialize() {
        ...
        $action = array(
            'permission' => array(),
            'allow' => array('listdialog', 'revert', 'adddialog', 'analytics', 'checkout', 'getmonthlyreceive', 'getyearreceivecomparison', 'getreceivablesmoney', 'getpayablesmoney', 'receivablesplan', 'advance_search', 'getcurrentstatus', 'index')
        );
        $this->type = $_REQUEST['t'] ? trim($_REQUEST['t']) : 'receivables';
        if (!in_array($this->type, array('receivables', 'payables', 'receivingorder', 'paymentorder', 'receivablesplan', 'index'))) {
    ...
    public function index_beifen() {
        ...
        switch ($by) {
            case 'create' :
                $where[$this->type . '.creator_role_id'] = session('role_id');
                break;
            case 'sub' :
                $where[$this->type . '.owner_role_id'] = array('in', implode(',', $below_ids));
                break;
            case 'subcreate' :
                $where[$this->type . '.creator_role_id'] = array('in', implode(',', $below_ids));
                break;
            case 'none' :
                $where[$this->type . '.status'] = array('eq', 0);
                break;
            case 'part' :
                $where[$this->type . '.status'] = array('eq', 1);
                break;
            ...
            case 'month' :
                ...
                break;
            case 'deleted' :
                $where[$this->type . '.is_deleted'] = 1;
                break;
            case 'add' :
                $order = $this->type . '.create_time desc';
                break;
            case 'update' :
                $order = $this->type . '.update_time desc';
                break;
            case 'me' :
                $where[$this->type . '.owner_role_id'] = session('role_id');
                break;
            default :
            ...
            if ($field == "receivables.customer_id") {
                ...
                switch ($_REQUEST['condition']) {
                    ...
                    case "nbetween" :
                        ...
                        break;
                    case "tgt" :
                        $where[$field] = array('gt', $search + 86399);
                        break;
                    default :
                        $where[$field] = array('eq', $search);
                }
        ...
        switch ($this->type) {
            case 'receivables'  :
                $achievement = D("AchievementView");
                $list = $achievement->select();
                $this->assign('list', $list);
                $this->display('receivables');
            case 'receivables_beifen' :
                //应收款
...
//        }
        ...
        $count = $achievement->where($where_source)->count();
        $p_num = ceil($count / $listrows);
        if ($p_num < $p) {
            $p = $p_num;
        }

        $list = $achievement->where($where_source)->page($p . ',' . $listrows)->select();
        import("@.ORG.Page");
        $Page = new Page($count, $listrows);
        $Page->parameter = implode('&', $params);
    ...
    public function index99() {
        $achievement = D("AchievementView");
        $this->achievement = $achievement->select();
    ...
    public function reach() {
        ...
        ApiClient::init();
        $contactService = new com\hlw\huiliewang\interfaces\finance\ReachServiceClient(null);
        ApiClient::build($contactService);
        $contactRequestDo = new com\hlw\huiliewang\dataobject\finance\reachRequestDTO();
        if ($isExport) {
            $page = 1;
            $pageSize = 200;
        }
        $contactRequestDo->day = $day;
        $contactRequestDo->department = $department;
        $contactRequestDo->type = $type;
        $contactRequestDo->name = $name;
        $contactRequestDo->page = $page;
        $contactRequestDo->pageSize = $pageSize;

        $res = $contactService->listReach($contactRequestDo);
        $message = json_decode($res->message, true);
        if ($isExport) {
            $list = $message['list'];
            $expCellName = [
                ['full_name', '顾问'],
                ...
                ['achievement', '原始业绩'],
                ['rank', '职级'],
                ['业务形态', 'profession_type'],
                ['入职时期', 'entry'],
            ];
            return $this->exportExcel('业绩达成率数据', $list, $expCellName);
        }
    ...
    public function weekExport() {
        ...
        $fileData = [
            ...
            'create_date' => time(),
            'file_type' => 1,
            'data_date' => strtotime($fileDate),
            'file_path' => $info[0]['savepath'] . $info[0]['savename']
        ];
        $res = M('file')->add($fileData);
        if (!$res) {
            $return = ['success' => false, 'code' => 500, 'info' => "操作失败"];
            $this->ajaxReturn($return);
        }
        $fileData = [];
        $fileData['file_path'] = "./Uploads/temp/weekday.xls";
        $fileDate = '20190601';
        import('@.ORG.ApiClient');
        ApiClient::init();
        $reachService = new com\hlw\huiliewang\interfaces\finance\ReachServiceClient(null);
        ApiClient::build($reachService);
        $reachRequestDo = new com\hlw\huiliewang\dataobject\finance\reachRequestDTO();

        $file = realpath($fileData['file_path']);
        $file = str_replace("\\", "/", $file);
        $reachRequestDo->path = $file;
        $reachRequestDo->dataDate = $fileDate;
        $reachRequestDo->file_id = 1;
        $res = $reachService->weekdayExport($reachRequestDo);
        $message = $res->message;
     ...
     * @param $data  数据
     */
    function exportFamilyExcel($expTitle = '', $data) {
        if (count($data) == 0) {
            exit();
        }
        $xlsTitle = iconv('utf-8', 'gb2312', $expTitle);//文件名称
        $fileName = $xlsTitle . time();//or $xlsTitle 文件名称可根据自己情况设定
        $title = ['优先权日', '同族专利数', '优先权日', '申请(专利权)人', '公开(公告)号', '公开(公告)日', '技术类型', '专利名称'];
        $cellNum = count($title);
        ...
        //1、创建title
        $size = [12, 10, 12, 25, 16, 16, 18, 200];
        for ($i = 0; $i < $cellNum; $i++) {
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[$i] . '2', $title[$i]);
            $objPHPExcel->getActiveSheet()->getColumnDimension($cellName[$i])->setWidth($size[$i]);
            drawBorder($objPHPExcel, $cellName[$i] . '2'); //边框
            $objPHPExcel->getActiveSheet()->getStyle($cellName[$i] . '2')->getFont()->setBold(true); //字体加粗
        }
        //2、数据填充
        $index = 3; //深度
        foreach ($data as $list) {
            // 居中显示
            $objPHPExcel->setActiveSheetIndex(0)->getStyle('A')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $objPHPExcel->setActiveSheetIndex(0)->getStyle('B')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $objPHPExcel->setActiveSheetIndex(0)->getStyle('C')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $objPHPExcel->setActiveSheetIndex(0)->getStyle('D')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);

            //设置'优先权日', '同族专利数'（A3,B3）
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[0] . $index, $list['minpriorityfilingdate']);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[1] . $index, $list['familysize']);
            $h1 = 0;
            $startMer1 = $index;
            foreach ($list['subList'] as $info) {

                //设置'优先权日', '申请(专利权)人'（C3,D3）
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[2] . $index, $info['minpriorityfilingdate']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[3] . $index, $info['applicantnames']);
                $h = 0;
                $startMer = $index;
                foreach ($info['patents'] as $k => $patent) {
                    // '公开(公告)号', '公开(公告)日', '技术类型', '专利名称''（E3,F3,G3,H3）
                    $patent['typeids'] = change_type($patent['typeids']);
                    if (!$patent['title']) {
                        $patent['title'] = '';
                    }
                    $k > 0 && $index += 1;
                    $h++;
                    $h1++;
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[4] . $index, $patent['publicationkey']);
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[5] . $index, $patent['publicationdate']);
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[6] . $index, $patent['typeids']); //转换
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[7] . $index, $patent['title']);
                    //给单元格加边框
                    for ($cellCount = 0; $cellCount < count($title); $cellCount++) {
                        drawBorder($objPHPExcel, $cellName[$cellCount] . $index);
                    }
                }
                //合并
                if (isset($h) && $h > 1) {
                    $high = $h - 1;
                    $objPHPExcel->getActiveSheet(0)->mergeCells('C' . $startMer . ':' . 'C' . ($startMer + $high));//合并单元格
                    $objPHPExcel->getActiveSheet(0)->mergeCells('D' . $startMer . ':' . 'D' . ($startMer + $high));//合并单元格
                }
                $index += 1;
            }
            //合并
            if (isset($h1) && $h1 > 1) {
                $high1 = $h1 - 1;
                $objPHPExcel->getActiveSheet(0)->mergeCells('A' . $startMer1 . ':' . 'A' . ($startMer1 + $high1));//合并单元格
                $objPHPExcel->getActiveSheet(0)->mergeCells('B' . $startMer1 . ':' . 'B' . ($startMer1 + $high1));//合并单元格
            }
        }
