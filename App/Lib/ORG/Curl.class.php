...
class Curl {
    ...
    private static $content_type = 0;
    private static $returnCookie = 0; // 是否返回cookie; 默认0不返回

    const CONTENT_TYPE_JSON = 1;
    const CONTENT_TYPE_FORM_DATA = 2;
    const CONTENT_TYPE_FORM_URLENCODE = 3;

    /*
     * 发送请求入口
     * * */

    public static function send($url, $data = array(), $method = 'get', $cookie = '', $returnCookie = 0, $content_type = 0, $header = []) {
        //判断传入的url
        if (!$url) {
        ...
        self::$data = $data;
        self::$method = $method;
        self::$oriUrl = 'http://www.zhanjob.com'; //助手函数request获取当前url
        self::$cookie = $cookie;
    ...
    private static function doRequest($method) {
        ...
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1); //要求结果为字符串且输出到屏幕上curl_setopt($ch,CURLOPT_COOKIESESSION,true);  
        ...
        if ($method == 'post') {
            ...
            switch (self::$content_type) {
                case self::CONTENT_TYPE_FORM_URLENCODE :
                ...
                case self::CONTENT_TYPE_JSON:
                    curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode(self::$data));
                    break;
                default :
                    curl_setopt($curl, CURLOPT_POSTFIELDS, self::$data);
            }
